...
const char* SUBSCRIPTION = "emile/bed/#";
...
NeoPixelBus<NeoGrbwFeature, NeoSk6812Method> strip(pixelCount);
...
void setupOTA(){
  ArduinoOTA.setHostname("esp8266-leds");
...
void showStrip(uint8_t brightness = 100) {
  RgbwColor stripBackup[pixelCount];
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if(topicStr == "emile/bed/saturation") {
    saturationUpdated = true;
    saturation = value;
  }
  else if(topicStr == "emile/bed/brightness") {
    if(value <= 100) {
...
void restartIfClientDisconnected() {
  ...
  if(!client.connected()){
    ...
    while(!client.connected()){
      ...
      String clientId = "esp8266BedLeds-";
      clientId += String(random(0xFFFF), HEX);
      client.connect(clientId.c_str(), MQTT_USER, MQTT_PASSWORD, "emile/bed", 0, 0, "disconnected");
...
void setup() {
  ...
  client.publish("online", "ledsEmileEsp8266"); 
